...
void PrintOutput(std::shared_ptr<raft::objfactory<raft::server>> factory, int id, const std::string &func,
    Args &&...                                                   args)
{
        ...
        switch (m_state) {
            case State::Leader: {
                ...
                for (const auto &id : m_factory->GetAllObjKey()) {
                    if (id == (int)m_id || id < 0 || id >= (int)m_next_index_vec.size())
                        continue;

                    const auto &next_index = m_next_index_vec[id];
                    args.pre_log_index     = next_index - 1;
                    args.pre_log_term      = args.pre_log_index >= 0 && args.pre_log_index < (int)m_log_vec.size()
                    ...
                    thread_pool::get(0).submit([tmp, args] { tmp->RequestAppendEntries(args); });
                }
            }
            break;
            case State::Candidate: break;
            case State::Folower: {
                // 心跳计数，超时则触发选举
                if (++m_heartbeat == 6) {
                    m_heartbeat = 0;
...
void raft::server::Election()
{
    while (true) {
        ...
        // 发起请求投票
        const VoteArgs &args{m_term, (int)m_id, (int)m_log_vec.size() - 1,
...
void raft::server::ReplyAppendEntries(const AppendEntriesReply &reply)
{
    ...
    if (reply.success) {
        ...
        if (reply.log_count > 0)
            ...
            " ",
            m_next_index_vec[reply.id],
            " count:",
            reply.log_count);

        // 添加成功，更新跟随者的提交进度和同步进度
        m_match_index_vec[reply.id] = std::max(m_match_index_vec[reply.id],
...
void raft::server::AddPrint(const std::string &str)
{
    ...
    m_print_map[std::chrono::duration_cast<std::chrono::microseconds>(
        std::chrono::system_clock().now().time_since_epoch()).count()].push(str);
}
