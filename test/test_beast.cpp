...
handle_request(beast::string_view doc_root,
  ...
  static auto const bad_request = [&req](beast::string_view why) {
  ...
  static auto const not_found = [&req](beast::string_view target) {
  ...
  static auto const server_error = [&req](beast::string_view what) {
...
DOCTEST_TEST_CASE("test_beast,one") {
  ...
  co_spawn(ioc, do_listen(net::ip::tcp::endpoint{address, port}, doc_root),
           [](std::exception_ptr e) {
             if (e) {
               try {
                 std::rethrow_exception(e);
...
using shared_stream =
    std::shared_ptr<ssl::stream<boost::asio::ip::tcp::socket>>;

asio::awaitable<void> session_ssl(shared_stream stream) {
  beast::error_code ec;
  beast::flat_buffer buffer;
  stream.get()->set_verify_mode(
  ...
  try {
    for (;;) {
      ...
      // ??? Lambda ?????????????????????????????
      auto create_response = [&req]() -> http::response<http::string_body> {
        http::response<http::string_body> res{http::status::ok, req.version()};
        res.set(http::field::server, "Beast Example");
        ...
        if (req.target() == "/hello") {
          ...
          res.body() =
              R"({"message": "Hello from JSON!"})"; // Raw string literal
        } else {
          res.set(http::field::content_type, "text/html");
...
asio::awaitable<void> listener_ssl(net::ip::tcp::endpoint endpoint,
                                   std::shared_ptr<std::string const> doc_root,
                                   ssl::context ctx) {
  try {
    auto executor = co_await net::this_coro::executor;
    ...
    for (;;) {
      try {
        ...
        auto stream =
            std::make_shared<ssl::stream<tcp::socket>>(std::move(socket), ctx);
        co_await stream->async_handshake(ssl::stream_base::server,
                                         asio::use_awaitable);
        asio::co_spawn(executor, session_ssl(stream),
                       net::detached); // ??????? spawn ???Э??
      } catch (const std::exception &e) {
...
DOCTEST_TEST_CASE("test_beast,ssl_run") {
  try {
    ...
    const char *cert_file =
        "/mnt/h/study/cpp/fast_db_master/run/certificate.pem";
    const char *cert_private_file =
        "/mnt/h/study/cpp/fast_db_master/run/private.pem";

    asio::io_context io_context(1);

    ssl::context ctx(ssl::context::tlsv12);
    ctx.use_certificate_file("/mnt/h/study/cpp/fast_db_master/run/server.crt",
                             ssl::context::pem); // ??????
    ctx.use_private_key_file("/mnt/h/study/cpp/fast_db_master/run/server.key",
                             ssl::context::pem); // ?????

    asio::co_spawn(io_context,
                   listener_ssl(net::ip::tcp::endpoint{address, port}, doc_root,
                                std::move(ctx)),
                   [](std::exception_ptr e) {
                     if (e) {
                       try {
