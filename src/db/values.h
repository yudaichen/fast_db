...
public:
  ...
  void remove(const std::string &item) { items.remove(item); }
...
public:
  ...
  void add(const std::string &item) { items.insert(item); }

  void remove(const std::string &item) { items.erase(item); }
...
public:
  ...
  void add(double lat, double lon) { points.insert({lat, lon}); }
...
public:
  ...
  StringValue(std::string val) : value(std::move(val)) {}
...
public:
  ...
  void print() const override { std::cout << "Number: " << value << std::endl; }
...
public:
  ...
  // 设定键的值
  void set_value(const std::string &key, ValuePtr value) { data[key] = value; }

  // 获取键的值
  ValuePtr get_value(const std::string &key) {
    // 检查过期时间
    auto it = expiration_times.find(key);
    if (it != expiration_times.end() &&
  ...
  bool hset(const std::string &key, const std::string &field,
    ...
    if (it != data.end()) {
      ...
      if (const auto map_value =
              std::dynamic_pointer_cast<MapValue>(it->second)) {
        map_value->set(field, value); // 设置字段的值
        return true;
      } else {
        std::cerr << "Error: The key does not refer to a MapValue."
                  << std::endl;
      }
    } else {
...
public:
  ...
  static std::vector<std::string> parse_request(const std::string &data) {
    ...
    while ((end_pos = data.find("\r\n", pos)) != std::string::npos) {
      parsed.push_back(
          data.substr(pos + 1, end_pos - pos - 1)); // 去掉前面的 "+"
      pos = end_pos + 2;
    }
...
public:
  RedisSession(asio::ip::tcp::socket socket,
      ...
      : socket(std::move(socket)), slots(slots) {
    // 注册命令与处理函数的映射
...
private:
  ...
  asio::awaitable<void>
  process_command(const std::vector<std::string> &command) {
    if (command.empty()) {
    ...
    if (auto it = command_map_.find(cmd);
        it != command_map_.end() && command.size() >= 2) {
  ...
  // SET 命令处理
  asio::awaitable<void> set_value(const std::string &key,
                                  const std::string &value) {
  ...
  asio::awaitable<void> get_value(const std::string &key) {
    ...
    if (value_ptr) {
      co_await send_response("$" + std::to_string(value_ptr->type().size()) +
                             "\r\n" + value_ptr->type() + "\r\n");
    } else {
  ...
  // HSET 命令处理
  asio::awaitable<void> hset_value(const std::string &key,
  ...
  // HGET 命令处理
  asio::awaitable<void> hget_value(const std::string &key,
                                   const std::string &field) {
