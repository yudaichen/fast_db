...
struct [[nodiscard]] pr_task
{
    ...
    explicit pr_task(task_t &&task_,
        const priority_t priority_ = 0) noexcept(std::is_nothrow_move_constructible_v<task_t>) : task(std::move(task_)),
        priority(priority_) {}
...
template <typename T1, typename T2>
struct common_index_type<T1, T2, std::enable_if_t<
        (std::is_signed_v<T1> && std::is_unsigned_v<T2>) || (std::is_unsigned_v<T1> && std::is_signed_v<T2>)>>
{
    ...
    using type = std::conditional_t<larger_size <= 4,
        ...
        std::conditional_t<larger_size == 1 || (sizeof(S) == 2 && sizeof(U) == 1), std::int16_t, std::conditional_t<
            larger_size == 2 || (sizeof(S) == 4 && sizeof(U) < 4), std::int32_t, std::int64_t>>,
...
public:
     ...
     */
    template <BS_THREAD_POOL_INIT_FUNC_CONCEPT(F)>
    thread_pool(const std::size_t num_threads, F &&init) { create_threads(num_threads, std::forward<F>(init)); }
    ...
    void detach_blocks(const T1 first_index, const T2 index_after_last, F &&block, const std::size_t num_blocks = 0,
        const priority_t priority = 0)
    {
        if (static_cast<T>(index_after_last) > static_cast<T>(first_index)) {
            const std::shared_ptr<std::decay_t<F>> block_ptr = std::make_shared<std::decay_t<
                F>>(std::forward<F>(block));
            ...
            for (std::size_t blk = 0; blk < blks.get_num_blocks(); ++blk) {
                detach_task(
                    [block_ptr, start = blks.start(blk), end = blks.end(blk)] { (*block_ptr)(start, end); },
                    priority);
            }
        }
    }
     ...
     */
    template <typename T1, typename T2, typename T = common_index_type_t<T1, T2>, typename F>
    void detach_loop(const T1 first_index, const T2 index_after_last, F &&loop, const std::size_t num_blocks = 0,
        const priority_t priority = 0)
    {
        if (static_cast<T>(index_after_last) > static_cast<T>(first_index)) {
     ...
     * @param num_threads The number of threads to use.
     */
    void reset(const std::size_t num_threads) { reset(num_threads, [](std::size_t) {}); }
     ...
     */
    template <BS_THREAD_POOL_INIT_FUNC_CONCEPT(F)>
    void reset(F &&init) { reset(0, std::forward<F>(init)); }
    ...
    void reset(const std::size_t num_threads, F &&init)
    {
        if constexpr (pause_enabled) {
            ...
            tasks_lock.lock();
            paused = was_paused;
        } else { reset_pool(num_threads, std::forward<F>(init)); }
    }
    ...
    {
        if constexpr (std::is_invocable_v<F, std::size_t>) { cleanup_func = std::forward<F>(cleanup); } else {
     ...
     */
    template <typename T1, typename T2, typename T = common_index_type_t<T1, T2>, typename F, typename R =
        std::invoke_result_t<std::decay_t<F>, T, T>>
     ...
     */
    template <typename T1, typename T2, typename T = common_index_type_t<T1, T2>, typename F, typename R =
        std::invoke_result_t<std::decay_t<F>, T>>
    [[nodiscard]] multi_future<R> submit_sequence(const T1 first_index, const T2 index_after_last, F &&sequence,
        const priority_t priority = 0)
    {
        if (static_cast<T>(index_after_last) > static_cast<T>(first_index)) {
            const std::shared_ptr<std::decay_t<F>> sequence_ptr = std::make_shared<std::decay_t<F>>(
...
#define BS_THREAD_POOL_PROMISE_MEMBER_ACCESS promise.
        ...
        detach_task(
            [task = std::forward<F>(task), promise = std::move(promise)]() mutable {
                ...
                try {
                    ...
                    } else { BS_THREAD_POOL_PROMISE_MEMBER_ACCESS set_value(task()); }
#ifdef __cpp_exceptions
                } catch (...) {
                    try { BS_THREAD_POOL_PROMISE_MEMBER_ACCESS set_exception(std::current_exception()); } catch (...) {}
                }
#endif
            },
            priority);
    ...
    bool wait_for(const std::chrono::duration<R, P> &duration)
    {
        ...
        std::unique_lock tasks_lock(tasks_mutex);
        waiting = true;
        const bool status = tasks_done_cv.wait_for(tasks_lock,
            duration,
            [this] {
                if constexpr (pause_enabled)
                    return (tasks_running == 0) && (paused || tasks.empty());
    ...
    bool wait_until(const std::chrono::time_point<C, D> &timeout_time)
    {
        ...
        std::unique_lock tasks_lock(tasks_mutex);
        waiting = true;
        const bool status = tasks_done_cv.wait_until(tasks_lock,
            timeout_time,
            [this] {
                if constexpr (pause_enabled)
                    return (tasks_running == 0) && (paused || tasks.empty());
...
private:
    ...
    {
        if constexpr (std::is_invocable_v<F, std::size_t>) { init_func = std::forward<F>(init); } else {
    ...
    void worker(BS_THREAD_POOL_WORKER_TOKEN const std::size_t idx)
    {
        ...
        while (true) {
                ...
                break;
            {
                ...
                // NOLINT(misc-const-correctness) In C++23 this cannot be const since `std::move_only_function::operator()` is not a const member function.
                ++tasks_running;
                tasks_lock.unlock();
#ifdef __cpp_exceptions
                try {
...
public:
     ...
     */
    template <typename... T>
    explicit synced_stream(T &... streams) { (add_stream(streams), ...); }
     ...
     */
    template <typename... T>
    void println(T &&... items) { print(std::forward<T>(items)..., '\n'); }
